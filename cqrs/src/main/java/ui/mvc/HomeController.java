...
public class HomeController implements Controller {
	...
	public View changeName(Map params) {
		...
		//
		bus.send(new RenameStock(id, newName, version));
	...
	public View deactivate(Map params) {
		UUID id = getId(params);
		int version = getVersion(params);
		bus.send(new UnlistStockFromExchange(id, version));
	...
	public View checkIn(Map params) {
		...
		bus.send(new AddStocktoPortFolio(id, count, version));
	...
	public View remove(Map params) {
		...
		bus.send(new RemoveStockfromPortfolio(id, count, version));
